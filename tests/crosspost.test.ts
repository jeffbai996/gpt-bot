...
test('crosspost delivery leaves the source quiet, regardless of final wording', () => {
  ...
  assert.equal(shouldQuietCrosspostReceipt('Replied in chat… was auto-generated by the relay.', posts), true)
  assert.equal(shouldQuietCrosspostReceipt('The detailed answer is also in #chat.', posts), true)
  assert.equal(shouldQuietCrosspostReceipt(receipt, []), false)
  assert.equal(shouldQuietCrosspostReceipt('The detailed answer is also in #chat.', []), false)
})
