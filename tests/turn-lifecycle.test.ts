...
test('narration mode controls retention and live reasoning preserves current progress', async () => {
  const source = await readFile(new URL('../src/gpt.ts', import.meta.url), 'utf8')
  ...
  assert.doesNotMatch(reasoningBranch, /narrationHistory\.clearCurrent\(\)/)
  ...
  assert.match(source, /narrationBlocks\(text, flags\.thinking !== 'live'\)/)
  assert.match(source, /quoteDetail: flags\.thinking !== 'live'/)
})
